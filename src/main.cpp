...
const unsigned long RETARDO_ANTI_REBOTE_SW = 100; 
...
int alarmaCO2Min = 600;
int alarmaCO2Max = 1200;
...
const int ALARMA_CO2_MIN_RANGO = 300;
const int ALARMA_CO2_MAX_RANGO = 4000;
const int ALARMA_CO2_PASO = 10;
...
const int CO2_DISPLAY_THRESHOLD = 10;     // Cambio de 10 ppm
  ...
  if (proximoEstado != estadoActualApp || necesitaRefrescarLCD) {
      if (proximoEstado != estadoActualApp) {
          ...
          if (proximoEstado == ESTADO_MENU_PRINCIPAL) {
              ...
              // No limpiamos la LCD aquí, lo hará manejarModoFuncionamiento para el mensaje inicial
          }
      ...
      // ETIQUETA: lcd.begin() se mantiene aquí según tu indicación para estabilidad
      lcd.begin(); 
      lcd.backlight(true); // Asegurar que la luz de fondo esté encendida
...
void manejarModoFuncionamiento(int deltaEncoder, bool pulsadoSwitch) { 
    ...
    if (!initialMessageDisplayed) {
        ...
        // No necesitamos `return` aquí, ya que el loop se encargará de esto con `delay(10)`
    }
    ...
    if (changed) {
        ...
        lcd.print("CO2: " + String(lastDisplayedCO2) + " ppm", 0, 2);
